...
```
ReferenceError: awaitproducts is not defined
```
...
> 8  | const products = page.locator('//div[@class="product-productMetaInfo"]').click();
     |                                                                          ^ Error: locator.click: Test ended.
  9  | //await product.click();
  10 | const productcount = awaitproducts.count();
  11 | console.log("total products are:",productcount);
  12 | const productlist = [];
  13 | for (let i=0;i< productcount;i++){
  14 |     const product = await products.nth(i);
  15 |     const title = await product.locator(".product-brand").textContent();
  16 |     const pricetext = await product.locator(".product-discountedPrice").textContent();
  17 |     const price = parseInt(pricetext.replace(/[^\d]/g,""));
  18 | 
  19 |     productlist.push({
  20 |         title,
  21 |         price
  22 |     });
  23 |     console.log(productlist);
  24 |     const miniproduct = productlist.reduce((min,current)=>current.price<min?current:min);
  25 |     console.log("Minimum product:", miniproduct);
  26 |     const maxproduct = productlist.reduce((max,current)=>max.price>current.price?max:current);
  27 |     console.log("Maximum product:", maxproduct);
  28 | }
  29 | // const products = page.locator('//div[@class="product-productMetaInfo"]');
  30 | // const count =await products.count();
  31 | // console.log(count);
  32 | // const productlist = [];
  33 | // for (let i=0;i<count;i++){
  34 | //     const product=products.nth(i);
  35 | //     const title = await product.locator(".product-brand").textContent();
  36 | //     await page.waitForTimeout(2000);
  37 | // const pricetext =await product.locator(".product-discountPercentage").textContent();
  38 | // await page.waitForTimeout(2000);
  39 | // const price = parseInt(pricetext.replace(/[^\d]/g,""));
  40 | // productlist.push({
  41 | //     title,
  42 | //     price
  43 | 
  ...
  45 |  
  46 | // }
  47 | // console.log(productlist);
  48 | // const miniproduct = productlist.reduce((min,current)=>current.price<min?current:min
  49 | // );
  50 | // const maximum_price = productlist.reduce((max,current)=>max.price > current.price? max:current
  51 | // );
  52 | // console.log("minimum amount is:",miniproduct);
  53 | // console.log("maximum price is:",maximum_price);
  54 | $
  55 | });
  56 | 
  57 | 
