...
```ts
  ...
  4  |    
> 5  |     await page.locator('#twotabsearchtextbox').fill("iphone").page.down()
     |                                                ^ Error: locator.fill: Test ended.
  6  |      await page.waitForTimeout(3000)
  7  |      
  8  |      await page.locator('//div[@id="sac-suggestion-row-1"]').click()
  9  | 
  10 |     // await page.locator('#twotabsearchtextbox').press('Enter');
  11 |     // await page.waitForURL(/\/s\?.*field-keywords=iphone/);
  12 | })
```